...
from PacMan.bossSave import bossSave
...
def menuWork(game, score, lives):
    ...
    ghost = pygame.Rect(30, 30, 15, 15)
    ghostC = pygame.Rect(470, 430, 15, 15)
    ghostP = pygame.Rect(470, 30, 15, 15)
    ghostI = pygame.Rect(30, 430, 15, 15)
    player = pygame.Rect(230, 170, 15, 15)
    ghostCopy = pygame.Rect(ghostP.x, ghostP.y, 15, 15)
    ...
    positionA = []
    positionB = []
    positionA.append('n24')
    positionB.append('n24')
    normalBool = True
    #randomNumber = 0
    ...
    def getRandom(randomNumber):
        
        randomNumber = random.randint(1, 63)
        return randomNumber
    ...
    while True:
        ...
        if gameTimer % 200 == 0 and eatenC == False:
            xC = 0
            xY = 0
            value_list = list(nodes.values())
            randomPos2 = value_list[rand]
            ghostCPos = randomPos2
            ghostC.x = ghostCPos.x
            ghostC.y = ghostCPos.y
    ...
    #same as above but for inky
        ...
        for i in range(len(pathCopy)):
            if i >= 1:
                if ghostCopy.x != next_spotCopy.x:
                    if ghostCopy.x > next_spotCopy.x:
                        xCopy = -1.3
                        yCopy = 0
                    if ghostCopy.x < next_spotCopy.x:
                ...
                if ghostCopy.y != next_spotCopy.y:
                    if ghostCopy.y > next_spotCopy.y:
                        yCopy = -1.3
                        xCopy = 0
                    if ghostCopy.y < next_spotCopy.y:
            ...
            #this is for chasing pac man between two nodes when path is empty
            if i < 1:
                if ghostCopyPos == playerPos:
                    if ghostCopy.x > player.x:
                        xCopy = -1.3
                        yCopy = 0
                    if ghostCopy.x < player.x:
                        xCopy = 2
                        yCopy = 0
                    if ghostCopy.y > player.y:
                        yCopy = -1.3
                        xCopy = 0
                    if ghostCopy.y < player.y:
                        yCopy = 2
        ...
        if eatenC == False:
            for j in nodes:
                ...
                if frightenedC == False:
                    if ghostC.contains(nodes[j]):
        ...
        for i in nodes:
            if ghost.contains(nodes[i]) or ghostP.contains(nodes[i]):
        ...
        if player.colliderect(nb) or player.colliderect(ni):
            player.y = player.y - 2
        ...
        #if:#
            #copies.append(ghostCopy)#

        #if player.colliderect:#
            #copies.remove(ghostCopy)

    
        #for i in range(len(copies)):
            #windowSurface.blit(pinkyStretchedChase, copies[i])
        ...
        for wall in walls:
            pygame.draw.rect(windowSurface, RED, wall.rect)
        ...
        #frightened mode
        if frightenedB == True and playerChange < 150:
        ...
        if frightenedC == True and playerChange < 150:
            windowSurface.blit(frightenedStretch, ghostC)
            if ghostC.colliderect(player):
                eatenC = True
                eatMusic.play()
        ...
        if frightenedP == True and playerChange < 150:
            windowSurface.blit(frightenedStretch, ghostP)
            if ghostP.colliderect(player):
                eatenP = True
                eatMusic.play
        ...
        if frightenedI == True and playerChange < 150:
            windowSurface.blit(frightenedStretch, ghostI)
            if ghostI.colliderect(player):
                eatenI = True
                eatMusic.play()
        ...
        if frightenedB == True and eatenB == False and changeMusic == True:
